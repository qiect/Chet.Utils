---
title: "TaskHelper"
description: "TaskHelper 是一个静态帮助类，为异步任务提供了丰富的管理功能，包括任务创建、任务等待、超时控制、重试机制、断路器、并行处理、任务限流、任务队列、任务池、任务监控、任务取消、任务结果处理、任务异常处理、任务协调等，旨在简化复杂的异步编程场景，提高代码的可读性和可维护性。"
---
