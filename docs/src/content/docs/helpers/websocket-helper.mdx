...
description: "WebSocketHelper 是一个 WebSocket 客户端和服务器辅助类，为 WebSocket 通信提供了丰富的功能，包括连接管理、消息收发、心跳检测、自动重连、消息队列、事件驱动模型、线程安全操作、自动资源管理等，旨在封装 WebSocket 的复杂性，使 WebSocket 的使用更加简单和可靠。"
---
